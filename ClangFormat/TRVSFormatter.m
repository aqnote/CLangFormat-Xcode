...
static TRVSFormatter *sharedFormatter;
...
+ (instancetype)sharedFormatter {
  ...
  dispatch_once(&onceToken, ^{
    sharedFormatter = [[self alloc] initWithStyle:nil exec:nil isSysExec:NO];
  });
...
- (instancetype)initWithStyle:(NSString *)style
  ...
  if (self = [self init]) {
    self.style = style;
    self.exec = exec;
    self.isSysExec = isSysExec;
  }
  return self;
}

- (void)formatActiveFile {
  [self formatRanges:@[ [NSValue
                         valueWithRange:[TRVSXcode wholeRangeOfTextView]] ]
  ...
  [[TRVSXcode selectedFileNavigableItems]
      enumerateObjectsUsingBlock:^(IDEFileNavigableItem *fileNavigableItem,
        ...
        if ([document
          ...
          [self formatRanges:@[
            [NSValue
                valueWithRange:NSMakeRange(0, [[sourceCodeDocument textStorage]
                                                  length])]
          ]
...
- (void)formatRanges:(NSArray *)ranges
  ...
  if (![document trvs_shouldFormat]) return;
  ...
  void (^handler)(NSArray *, NSArray *) = ^(NSArray *fragments,
                                            NSArray *errors) {
    if (errors.count == 0) {
...
- (void)fragmentsOfContinuousLineRanges:(NSArray *)continuousLineRanges
  ...
  NSString *executablePath = self.exec;
  if (self.isSysExec) {
  ...
  [continuousLineRanges enumerateObjectsUsingBlock:^(
                            NSValue *rangeValue, NSUInteger idx, BOOL *stop) {
    ...
    [fragment formatWithStyle:self.style
                   launchPath:executablePath
                    lineRange:lineRange
                        block:^(NSString *formattedString, NSError *error) {
